...
import { useWiderrufStore, getStepIndex, getProgress } from "@/lib/widerrufsbelehrung/store";
import { WIZARD_STEPS, type WizardStep } from "@/lib/widerrufsbelehrung/types";
import { getCompletionStatus } from "@/lib/widerrufsbelehrung/contract";
import { Logo } from "@/components/brand/logo";
import { cn } from "@/lib/utils";
import { StepAnbieter } from "./steps/step-anbieter";
import { StepLeistung } from "./steps/step-leistung";
import { StepAusschluesse } from "./steps/step-ausschluesse";
import { StepRueckgabe } from "./steps/step-rueckgabe";
import { StepReview } from "./steps/step-review";

export function WizardShell() {
  const currentStep = useWiderrufStore((s) => s.currentStep);
  const setStep = useWiderrufStore((s) => s.setStep);
  const goNext = useWiderrufStore((s) => s.goNext);
  const goPrev = useWiderrufStore((s) => s.goPrev);
  const reset = useWiderrufStore((s) => s.reset);
  ...
  return (
    <div className="relative z-10 min-h-screen flex flex-col">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-14 pb-2 w-full" key={`title-${currentStep}`}>
        ...
        >
          Schritt {String(stepIdx + 1).padStart(2, "0")} · Widerrufsbelehrung · § 312g BGB
        </p>
        <h1
        ...
        {currentStep === "anbieter" && (
          <p className="mt-4 text-[12px] text-ink-faded rise" style={{ animationDelay: "110ms" }}>
            Pflichtangaben nach §§ 312g, 355, 356, 357 BGB + Anhang § 312f BGB (Muster-Widerrufsformular).
            Compliflow generiert die gesetzlich vorgeschriebenen Standard-Klauseln,{" "}
      ...
      <main id="main-content" className="flex-1 max-w-5xl mx-auto px-6 py-12 w-full">
        <div key={currentStep} className="rise" style={{ animationDelay: "120ms" }}>
          ...
          {currentStep === "leistung" && <StepLeistung />}
          {currentStep === "ausschluesse" && <StepAusschluesse />}
          {currentStep === "rueckgabe" && <StepRueckgabe />}
...
function missingHint(step: WizardStep): string {
  switch (step) {
    ...
    case "leistung":
      return "Leistungstyp + Frist (min. 14 Tage)";
    case "ausschluesse":
      return "";
    case "rueckgabe":
      return "Bei abweichender Adresse + Sperrgut: Felder ausfüllen";
    default:
      return "";
  }
}
