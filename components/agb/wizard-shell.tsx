...
import { useAgbStore, getStepIndex, getProgress } from "@/lib/agb/store";
import { WIZARD_STEPS, type WizardStep } from "@/lib/agb/types";
import { getCompletionStatus } from "@/lib/agb/contract";
import { Logo } from "@/components/brand/logo";
import { cn } from "@/lib/utils";
import { StepVariante } from "./steps/step-variante";
import { StepAnbieter } from "./steps/step-anbieter";
import { StepLeistung } from "./steps/step-leistung";
import { StepZahlung } from "./steps/step-zahlung";
import { StepLieferung } from "./steps/step-lieferung";
import { StepHaftung } from "./steps/step-haftung";
import { StepReview } from "./steps/step-review";

export function WizardShell() {
  const currentStep = useAgbStore((s) => s.currentStep);
  const setStep = useAgbStore((s) => s.setStep);
  const goNext = useAgbStore((s) => s.goNext);
  const goPrev = useAgbStore((s) => s.goPrev);
  ...
  return (
    <div className="relative z-10 min-h-screen flex flex-col">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-14 pb-2 w-full" key={`title-${currentStep}`}>
        ...
        >
          Schritt {String(stepIdx + 1).padStart(2, "0")} · AGB · §§ 305-310 BGB
        </p>
        <h1
        ...
        {currentStep === "anbieter" && (
          <p className="mt-4 text-[12px] text-ink-faded rise" style={{ animationDelay: "110ms" }}>
            Pflichtangaben nach §§ 305-310, 312i BGB + Art. 246 EGBGB. Compliflow generiert
            Standard-Klauseln nach BGB-Recht,{" "}
            <strong className="text-ink-dim">ersetzt aber keine Rechtsberatung</strong> bei
            Sonderfällen oder Mischverträgen.
          </p>
        )}
      </div>
      ...
      <main id="main-content" className="flex-1 max-w-5xl mx-auto px-6 py-12 w-full">
        <div key={currentStep} className="rise" style={{ animationDelay: "120ms" }}>
          {currentStep === "variante" && <StepVariante />}
          {currentStep === "anbieter" && <StepAnbieter />}
          {currentStep === "leistung" && <StepLeistung />}
          {currentStep === "zahlung" && <StepZahlung />}
          {currentStep === "lieferung" && <StepLieferung />}
          {currentStep === "haftung" && <StepHaftung />}
...
function missingHint(step: WizardStep): string {
  switch (step) {
    ...
    case "leistung":
      return "Leistungstyp + Beschreibung";
    case "zahlung":
      return "Mind. eine Zahlungsart wählen";
    case "lieferung":
      return "Versand-Felder bei Shop";
    default:
      return "";
  }
}
