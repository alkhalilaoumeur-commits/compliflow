...
import { useCookieBannerStore, getStepIndex, getProgress } from "@/lib/cookie-banner/store";
import { WIZARD_STEPS, type WizardStep } from "@/lib/cookie-banner/types";
import { getCompletionStatus } from "@/lib/cookie-banner/builder";
import { Logo } from "@/components/brand/logo";
import { cn } from "@/lib/utils";
import { StepAnbieter } from "./steps/step-anbieter";
import { StepKategorien } from "./steps/step-kategorien";
import { StepTracking } from "./steps/step-tracking";
import { StepStil } from "./steps/step-stil";
import { StepVerhalten } from "./steps/step-verhalten";
import { StepReview } from "./steps/step-review";

export function WizardShell() {
  const currentStep = useCookieBannerStore((s) => s.currentStep);
  const setStep = useCookieBannerStore((s) => s.setStep);
  const goNext = useCookieBannerStore((s) => s.goNext);
  const goPrev = useCookieBannerStore((s) => s.goPrev);
  ...
  return (
    <div className="relative z-10 min-h-screen flex flex-col">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-14 pb-2 w-full" key={`title-${currentStep}`}>
        ...
        >
          Schritt {String(stepIdx + 1).padStart(2, "0")} · Cookie-Banner · § 25 TDDDG
        </p>
        <h1
        ...
        {currentStep === "anbieter" && (
          <p className="mt-4 text-[12px] text-ink-faded rise" style={{ animationDelay: "110ms" }}>
            Einwilligungspflicht nach § 25 TDDDG + Art. 6/7 DSGVO. BGH-2025-konforme Reject-All-Prominenz
            wird automatisch erzwungen.{" "}
            <strong className="text-ink-dim">Compliflow ersetzt keine Rechtsberatung</strong> bei
            Sonderfällen (z.B. Drittland-Transfers, IAB TCF).
          </p>
        )}
      </div>
      ...
      <main id="main-content" className="flex-1 max-w-5xl mx-auto px-6 py-12 w-full">
        <div key={currentStep} className="rise" style={{ animationDelay: "120ms" }}>
          ...
          {currentStep === "kategorien" && <StepKategorien />}
          {currentStep === "tracking" && <StepTracking />}
          {currentStep === "stil" && <StepStil />}
          {currentStep === "verhalten" && <StepVerhalten />}
...
function missingHint(step: WizardStep): string {
  switch (step) {
    case "anbieter":
      return "Name + Datenschutz-URL Pflicht";
    case "kategorien":
      return "Essential muss aktiv sein";
    case "tracking":
      return "";
    case "stil":
      return "";
    case "verhalten":
      return "Laufzeit 1-24 Monate";
    default:
      return "";
  }
}
